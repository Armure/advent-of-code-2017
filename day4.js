...
function isInvalidPassphrase (phrase) {
  let duplicate = false
  const values = []
  phrase.forEach(word => {
    if (values.includes(word)) duplicate = true
    values.push(word)
  })
  return duplicate
}

function getValidPhrases (input) {
  let total = 0
  input.forEach(phrase => {
    const isInalid = isInvalidPassphrase(phrase)
    if (!isInalid) total++
  })
  return total
}
...
const total = getValidPhrases(formattedInput)
console.log(total)
