...
fInput.forEach(line => {
  ...
  const cName = parts[4]
  const cOperator = parts[5]
  const cValue = parseFloat(parts[6])

  let register = registers.find(r => r.name === name)
  if (!register) {
    register = { name, value: 0 }
    registers.push(register)
  }

  const cRegister = registers.find(r => r.name === cName)
  if (checkCondition(cRegister ? cRegister.value : 0, cOperator, cValue)) {
    if (instruction === 'inc') register.value += value
    else if (instruction === 'dec') register.value -= value
  }
})

function checkCondition (name, operator, value) {
  switch (operator) {
    case '>':
      return name > value
...
const values = registers.map(register => register.value)
