const input = `mmqyju (156) -> rjzvwv, noybkx
...
const fInput = input.split('\n').map(val => val.replace(/ *\([^)]*\) */g, ''))

const programs = fInput.map(program => {
  const split = program.split('->')
  const name = split[0]
  const holding = split[1] ? split[1].split(',').map(val => val.trim()) : []
  return { name, holding }
})

programs.forEach(({ name, holding }) => {
  holding.forEach(held => {
    const program = programs.find(p => p.name === held)
    program.parent = name
  })
})
